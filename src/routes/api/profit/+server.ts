...
import { getProfit } from '$lib/db/transactions';
import { z } from 'zod';
...
export const POST: RequestHandler = async ({ locals, request }) => {
    ...
    const profit = await getProfit(locals.user.id, form.data.start, form.data.end,);

    return new Response(devalue.stringify(profit));
};