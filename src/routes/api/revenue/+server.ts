...
import { getRevenue } from '$lib/db/transactions';
import { z } from 'zod';
...
export const POST: RequestHandler = async ({ locals, request }) => {
    ...
    const revenue = await getRevenue(locals.user.id, form.data.start, form.data.end,);

    return new Response(devalue.stringify(revenue));
};