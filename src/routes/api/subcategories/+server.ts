...
import { getSubcategories } from '$lib/db/transactions';
import { z } from 'zod';
...
export const POST: RequestHandler = async ({ locals, request }) => {
    ...
    const subcategories = await getSubcategories(locals.user.id, form.data.start, form.data.end,);

    return new Response(devalue.stringify(subcategories));
};